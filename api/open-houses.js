...
a// /api/open-houses.js â Vercel Serverless Function
// Scrapes Zillow for Miami-Dade open houses under $1M
// Applies Primesant ring/ZIP priority logic
...
async function getZillowCookies() {
  try {
    const resp = await fetch('https://www.zillow.com/miami-dade-county-fl/open-houses/', {
      method: 'GET',
...
async function fetchZillowPage(page = 1, cookies = '') {
  const searchQuery = {
    searchQueryState: {
  ...
  const headers = {
    ...
    'Accept': '*/*',
    'Accept-Language': 'en-US,en;q=0.9,es;q=0.8',
    'Referer': 'https://www.zillow.com/miami-dade-county-fl/open-houses/',
...
async function fetchAgentInfo(detailUrl, cookies = '') {
  try {
    ...
    const brokerMatch = html.match(/"brokerName"\s*:\s*"([^"]*?)"/);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // First get cookies from Zillow
    console.log('Getting Zillow cookies...');
    const cookies = await getZillowCookies();
    console.log('Cookies obtained:', cookies ? 'yes' : 'no');

    // Fetch all pages from Zillow
    let allListings = [];
    console.log('Fetching page 1...');
    const firstPage = await fetchZillowPage(1, cookies);
    const firstListings = extractListings(firstPage);
    allListings.push(...firstListings);
    console.log('Page 1: ' + firstListings.length + ' listings');
    ...
    console.log('Saturday (' + saturday + '): ' + satListings.length + ', Sunday (' + sunday + '): ' + sunListings.length);

    // Select best ZIPs (prioritize by ring first, then count)
    const satZip = selectBestZips(satListings, saturday);
    const sunZip = selectBestZips(sunListings, sunday, satZip);
    console.log('Selected ZIPs - Sat: ' + satZip + ', Sun: ' + sunZip);

    // Get top 5 for each day, sorted by time
    ...
    const agentPromises = allSelected.map(l => fetchAgentInfo(l.detailUrl, cookies));
